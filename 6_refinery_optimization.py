...
mod.q = pe.Var(all_items, initialize=0, domain=pe.NonNegativeReals)
...
# fuel_ratio = {'lo': 0.55, 'co': 0.22, 'ho': 0.17, 'r': 0.0555}
...
mod.cg_mass = pe.Constraint(expr=mod.q['cgpmf'] + mod.q['cgrmf'] - mod.q['cg'] == 0)
mod.rg_mass = pe.Constraint(expr=mod.q['rgpmf'] + mod.q['rgrmf'] - mod.q['rg'] == 0)
...
mod.jetfuel_pres_con = pe.Constraint(expr=sum(vape[i] * mod.q['%sjf' % i] for i in vape.keys()) - 1 * mod.q['jf'] <= 0)
